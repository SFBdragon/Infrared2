use crate::{Board, Sq, for_sq, PAWN_IDX, KING_IDX};

pub const DRAW: i16 = 0;
pub const MATE: i16 = -32000;
pub const UNCERTAIN_MATE: i16 = -31000;

pub const ACTV: usize = 0;
pub const IDLE: usize = 1;
pub const MG: usize = 0;
pub const EG: usize = 1;

impl Board {
    pub(super) fn eval_add(&mut self, add: [i16; 2]) {
        self.eval[MG] += add[MG];
        self.eval[EG] += add[EG];
    }
    pub(super) fn eval_sub(&mut self, sub: [i16; 2]) {
        self.eval[MG] -= sub[MG];
        self.eval[EG] -= sub[EG];
    }

    pub fn eval(&self) -> i16 {
        let mg = self.eval[0] as i32 *       self.phase as i32;
        let eg = self.eval[1] as i32 * (24 - self.phase as i32);
        let mut eval = (mg + eg) / 24;
    
        // small tempo bonus
        eval += 20;
        // awareness of upcoming draws to the 50 move rule
        eval = eval * 64.min(100 - self.fifty_move_clock as i32) / 64;
...
pub fn calc_eval_phase(pos: &Board) -> ([i16; 2], u8) {
    ...
    for piece in KING_IDX..=PAWN_IDX {
        for side in [ACTV, IDLE] {
            for_sq!(sq in pieces[piece] & masks[side] => {
                phase += PHASE_ADJUST[piece];
                for p in [MG, EG] {
                    eval[p] += PSQT[side][piece][sq.us()][p];
                }
            });
        }
    }
...
pub const PHASE_ADJUST: [u8; 6] = [0, 4, 2, 1, 1, 0];
