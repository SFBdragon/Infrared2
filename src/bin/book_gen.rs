...
fn main() {
    ...
    let mut graph = TreeBuilder::new()
        .with_root(Node::new(PosData { hash: 0, importance: 0 }))
        .build();
                ...
                if let Some((hashes, outcome)) = parse {
                    ...
                    for &hash in hashes.iter() {
                        ...
                        'children: for child_id in graph.children_ids(&node_id).unwrap() {
                            if graph.get(child_id).unwrap().data().hash == hash {
                        ...
                        if !found_child {
                            node_id = graph.insert(
                                Node::new(PosData { hash, importance: 0 }),
    ...
    for i in 1..positions.len() {
        if positions[i].hash == positions[i-1].hash {
            positions[writer].importance += positions[i].importance;
        } else {
            writer += 1;
    ...
    for pos in positions {
        ...
        book.write(&pos.importance.to_le_bytes()).unwrap();
    }
...
impl Visitor for MyVisitor {
    ...
    fn end_game(&mut self) -> Self::Result {
        match self.parse_failed {
            false => Some((std::mem::replace(&mut self.hashes, Vec::new()), match self.outcome {
