...
pub fn eval(pos: &Board) -> i16 {
    ...
    // mobility: lesser valuable attacker + equal value w/tempo are excluded
    // this imbalance serves to provide a tempo boost in active positions, but less in closed
    let mut actv_cvrd = fend::pawns_fend_actv(pos.pawns & pos.actv);
    let mut idle_cvrd = fend::pawns_fend_idle(pos.pawns & pos.idle);
    let mut temp_cvrd = 0;
    for_sq!(sq in pos.knights & pos.actv => {
        let (rank, ffile) = (sq as usize/8, fold_file(sq as usize%8));
        eval += lerp(PIECE_SQ[KNIGHT_IDX][rank][ffile], mg_eg);
        let fend = fend::knight_fend(sq);
        temp_cvrd |= fend;
        eval += popcntf(fend & !idle_cvrd & !pos.actv) * MOBILITY;
    });
    for_sq!(sq in pos.knights & pos.idle => {
        let (irank, ffile) = (7-sq as usize/8, fold_file(sq as usize%8));
        eval -= lerp(PIECE_SQ[KNIGHT_IDX][irank][ffile], mg_eg);
        let fend = fend::knight_fend(sq);
        idle_cvrd |= fend;
        eval -= popcntf(fend & !actv_cvrd & !pos.idle) * MOBILITY;
    });
    actv_cvrd |= temp_cvrd;
    for_sq!(sq in pos.bishops & pos.actv => {
        let (rank, ffile) = (sq as usize/8, fold_file(sq as usize%8));
        eval += lerp(PIECE_SQ[BISHOP_IDX][rank][ffile], mg_eg);
        let fend = fend::bishop_fend(sq, pos.all);
        temp_cvrd |= fend;
        eval += popcntf(fend & !idle_cvrd & !pos.actv) * MOBILITY;
    });
    for_sq!(sq in pos.bishops & pos.idle => {
        let (irank, ffile) = (7-sq as usize/8, fold_file(sq as usize%8));
        eval -= lerp(PIECE_SQ[BISHOP_IDX][irank][ffile], mg_eg);
        let fend = fend::bishop_fend(sq, pos.all);
        idle_cvrd |= fend;
        eval -= popcntf(fend & !actv_cvrd & !pos.idle) * MOBILITY;
    });
    ...
    for_sq!(sq in pos.rooks & pos.actv => {
        let (rank, ffile) = (sq as usize/8, fold_file(sq as usize%8));
        eval += lerp(PIECE_SQ[ROOK_IDX][rank][ffile], mg_eg);
        let fend = fend::rook_fend(sq, pos.all);
        temp_cvrd |= fend;
        eval += popcntf(fend & !idle_cvrd & !pos.actv) * MOBILITY;
    ...
    for_sq!(sq in pos.rooks & pos.idle => {
        let (irank, ffile) = (7-sq as usize/8, fold_file(sq as usize%8));
        eval -= lerp(PIECE_SQ[ROOK_IDX][irank][ffile], mg_eg);
        let fend = fend::rook_fend(sq, pos.all);
        idle_cvrd |= fend;
        eval -= popcntf(fend & !actv_cvrd & !pos.idle) * MOBILITY;
    ...
    actv_cvrd |= temp_cvrd;
    for_sq!(sq in pos.queens & pos.actv => {
        let (rank, ffile) = (sq as usize/8, fold_file(sq as usize%8));
        eval += lerp(PIECE_SQ[QUEEN_IDX][rank][ffile], mg_eg);
        let fend = fend::queen_fend(sq, pos.all);
        temp_cvrd |= fend;
        eval += popcntf(fend & !idle_cvrd & !pos.actv) * MOBILITY;
    });
    for_sq!(sq in pos.queens & pos.idle => {
        let (irank, ffile) = (7-sq as usize/8, fold_file(sq as usize%8));
        eval -= lerp(PIECE_SQ[QUEEN_IDX][irank][ffile], mg_eg);
        let fend = fend::queen_fend(sq, pos.all);
        idle_cvrd |= fend;
        eval -= popcntf(fend & !actv_cvrd & !pos.idle) * MOBILITY;
    });
    ...
    let actv_king_ring = fend::king_fend(pos.actv_king_sq);
    ...
    let idle_king_ring = fend::king_fend(pos.idle_king_sq);
    ...
    for_sq!(sq in actv_pawns => {
        ...
        eval += (rank - 1) as f32 * lerp(PAWN_RANK_SQRT, mg_eg).powf(2.0);
    ...
    for_sq!(sq in idle_pawns => {
        let (rank, file) = (sq / 8, (sq % 8) as usize);
        eval -= (6 - rank) as f32 * lerp(PAWN_RANK_SQRT, mg_eg).powf(2.0);
...
const MOBILITY: f32 = 14.0; // safe mobility bonus
const COVERED: f32 = 3.0; // tile fend bonus
const FEND: f32 = 8.0; // occupied tile fend extra bonus


// ~1: closed -> ~0: open
const KNIGHT_OPEN_POS    : [f32; 2] =  [100.0, -20.0];
const ROOK_OPEN_POS      : [f32; 2] = [-130.0,  80.0];
